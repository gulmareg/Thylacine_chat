...
class AnimalAssistantGUI:
    ...
    def render_user_input(self):
        ...
        user_input = st.chat_input("Ask Thyla something...")
        if user_input:
            if user_input.lower() == "goodbye":
                #st.warning("You have exited the chat. Shutting down the server...")
            ...
            self.messages.append({"role": "user", "content": user_input})  # Save the user's message.
            self.messages.append({"role": "ai", "content": response})  # Save the assistant's response.
            st.session_state.messages = self.messages  # Update the session state.
